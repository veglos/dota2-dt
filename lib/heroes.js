const HEROES_BY_ID = [
  ...
  { id: 138, name: "Muerta" },
  { id: 145, name: "Kez" },
  { id: 146, name: "Ringmaster" }
];

export const HEROES = HEROES_BY_ID
  .slice()
  .sort((a, b) => a.name.localeCompare(b.name));

const HERO_NAME_BY_ID = Object.fromEntries(HEROES_BY_ID.map((hero) => [hero.id, hero.name]));

export function getHeroNameById(heroId) {
  const id = Number(heroId);
  if (!Number.isFinite(id) || id <= 0) return "N/A";
  return HERO_NAME_BY_ID[id] || "Unknown Hero";
}
